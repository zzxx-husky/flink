...
//		System.out.println(taskDescription);
		...
		ratesPath = jobConfiguration.getString("policy.rates.path", "/data/share/project/nova/flink-1.4.1/build-target/rates/");
		currentWindowStart = status.getProcessingStart();
		
		//TODO socket
		try {
			socket = new Socket("proj103", 9999);
	...
	public void inputBufferConsumed(long timestamp, long deserialization, long processing, long numRecords) {
		...
		synchronized (status) {
			...
			if (timestamp - currentWindowStart > windowSize) {
				...
				*/
				String ratesFile = workerName.trim() + "-" + instanceId + "-" + epoch;

				try {
					//TODO socket
//					Files.write(ratesFile, rates, Charset.forName("UTF-8"));
					outSocket.println(ratesLine + "#" + ratesFile);
				} catch (Exception e) {
